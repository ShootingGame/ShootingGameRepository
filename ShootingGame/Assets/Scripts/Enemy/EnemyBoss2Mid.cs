...
public class EnemyBoss2Mid : MonoBehaviour
{
	// ヒットポイント
	...
	void Update(){
		...
		count++;
			...
			if(count >= 81){
			delta = delta * -1;
			if (transform.position != new Vector3 (2.5f, 2, 0)) {
				transform.position = Vector3.MoveTowards 
				(transform.position, new Vector3 (0, 2, 0), 5.0f);
				count = 0;
	...
	void OnTriggerEnter2D (Collider2D c)
	{
		...
		if(hp <= 0 )
		{
			...
			//TODO　Playerの動きを停止　|| 背景も含めてgame自体を停止する場合は setGamePause()を使用する
			//FindObjectOfType<Manager> ().setPlayerPause(true); //プレイヤーのみを停止する場合
			FindObjectOfType<Manager> ().isPause = true; //プレイヤーのみを停止する場合
